...
const Beepcard: React.FC<Props> = ({uid, selected, handleClick}) => {
  return (
    <>
    <button className='bg-slate-600 text-white border-slate-800 border-2 bg-white py-2 px-4 flex rounded-xl w-10/12 relative flex gap-2' onClick={handleClick}>
        <div className='font-semibold italic text-slate-400'> UID: </div>
        <div className='tracking-wide text-slate-600'> {uid} </div>
        { selected && (
