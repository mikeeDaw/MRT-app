...
const Beepcard: React.FC<Props> = ({uid, selected, handleClick}) => {
  return (
    <>
    <button key={Number(uid)} className={'border-slate-800 border-2 break-all items-center py-2 px-4 flex rounded-xl w-10/12 relative flex gap-2 ' + (selected ? 'bg-emerald-600' : 'bg-white ')} onClick={handleClick}>
        <div className={'font-semibold italic break-normal ' + (selected ? 'text-slate-200' : 'text-slate-400')}> UID: </div>
        <div className={'tracking-wide text-left ' + (selected ? 'text-white' : 'text-slate-600') }> {uid} </div>
        {/* { selected && (
