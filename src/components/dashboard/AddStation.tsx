    ...
    const addNewStation = async () => {
      ...
      if(!namePattern.test(statName) || !codePattern.test(statCode) || !latitude || !longitude ){
        ...
        const response = await fetch(`${endpoint}/station/add`, {
          ...
          body: JSON.stringify({name: statName.toUpperCase() , code:statCode,coordinates:{x:latitude, y:longitude},connected:chosenStat})
        }).then(async (jason) => {
            if(jason.status === 200){
                getTheStations()
                setChosenStat([])
    ...
    <motion.div>
      ...
      <div className="w-full h-full absolute top-0 left-0 z-0">
        ...
        >  
        {
          stations.map((stat) => {
            sKey+=1
            return (
            <Marker key={sKey} position={[Number(stat.coordinates.x), Number(stat.coordinates.y)]}>
              <Popup>
                { titleCase(stat.name) + ' Station' }
              </Popup>
            </Marker>
            )
          })
      ...
      <motion.div {...introPage} className={"absolute md:right-16 xl:right-28 top-24 bg-white z-10 p-5 pb-4 w-[320px] md:w-[370px] rounded-xl transition-all " + (addWindow ? 'right-2' : 'right-[-100%]')} style={{boxShadow: "0 0 10px -3px #474747"}}>
        <div className="flex flex-col" >
          {
            sm && (
              <button className='absolute right-6' onClick={()=>setAddWindow(!addWindow)}>
          ...
          <div className='max-h-[270px] overflow-y-scroll scrollbar-hide'>
            ...
            <input type="email" id="helper-text" aria-describedby="helper-text-explanation" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 mt-1" placeholder="Station Name" value={statName} onChange={(e) => setStatName(e.target.value)}></input>

            <input type="code" id="helper-text" aria-describedby="helper-text-explanation" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 mt-2" placeholder="Station Code" value={statCode} onChange={(e) => setStatCode(e.target.value.toUpperCase())}></input>
                ...
                stations.map((stat,index) => {
                  ...
                  return(
                    ...
                    <div className='w-1/3 px-1 mb-1.5'>
                      <button className={'border w-full rounded-full flex py-1.5 justify-start items-center gap-1.5 px-2 ' + (flag ? 'border-[#25a135] bg-[#b6ffbf]' : ' border-[#565d8d] bg-slate-200 ')} onClick={()=>{connectClick(String(stat.code),index)}} disabled={latitude ? false : true}>
                        <span className={'p-1.5 rounded-full ' + (flag ? 'bg-[#25a135]' : 'bg-slate-400' ) }></span>
                        <span className='text-xs truncate'> { titleCase(stat.name)}</span>
                      </button>
                    </div>
                    </>
                  )
        ...
        <div className="text-end mt-3">
            <button className="bg-[#202758] px-4 text-white py-1 rounded-xl" onClick={addNewStation}> Save </button>
        </div>
        </div>
      </motion.div>
    </ motion.div>
