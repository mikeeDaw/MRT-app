...
const InputField: React.FC<Props> = ({
  ...
  const renderIcon = () => {
    if (forImg) return icon;
    else return <span> {textIcon} </span>;
  };
    ...
    <>
      <div className="flex mt-1 mb-2 w-full">
        ...
        <input
          type={type}
          className={
            "rounded-none rounded-e-lg border text-gray-900 outline-0 block flex-1 min-w-0 w-full text-sm p-2 focus:border-[#202758] " +
            (error ? "border-red-400" : "border-gray-300") +
            (onlyRead ? " bg-slate-100" : "")
          }
