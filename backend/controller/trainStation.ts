...
export const makeStation = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    try {
        ...
        connected.forEach(async (codeAdd:string) => {
            await StationModel.updateOne({code:codeAdd}, {
                $push: {
                    connected: code
                }  
            })
...
export const updateStation = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    const {origCode ,name, code, connRem, connAdd, connStay, coordinates} = req.body;

    try {
        
        const updated = await updateStationByCode(origCode,{name,code,connected: [...connStay, ...connAdd],coordinates})
        ...
        if(connRem.length != 0){
            connRem.forEach( async (codeRem:string) => {
                await StationModel.updateOne({code:codeRem}, {
                    $pull : {
                        connected: code
                    }
                })
...
export const delStation = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
